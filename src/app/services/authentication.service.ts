...
export class AuthenticationService {
        ...
        map((response: Response) => {
          ...
          if (this.token) {
            ...
            localStorage.setItem('user',
              JSON.stringify({ email, token: this.token }));
          }
          return response;
        })
      );
  ...
  register(username: string, email: string, password: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/register`, { email, name: username, password }, { headers: this.headers } )
      .pipe(
        map((response: Response) => {
  ...
  sendPasswordResetEmail(email: string): Observable<any>  {
    ...
    return this.http.post(`${this.apiUrl}/password-reset-email`,
      { email, url }, { headers: this.headers } )
      .pipe(
        map((response: Response) => {
          return response;
        })
  ...
  resetPassword(newPassword: string, confirmedPassword: string, token: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/reset-password`, { password: newPassword,
      confirm_password: confirmedPassword, token }, { headers: this.headers } )
      .pipe(
        map((response: Response) => {
          return response;
        })
